...
import os
from data_loader.ground_truth import ground_truth_data
from data_loader.ground_truth import util
import numpy as np
import PIL
...
class DSprites(ground_truth_data.GroundTruthData):
  ...
  def __init__(self, data_path, latent_factor_indices=None):
    # By default, all factors (including shape) are considered ground truth
    # factors.
    dsprites_path = os.path.join(data_path, "dsprites",
    "dsprites_ndarray_co1sh3sc6or40x32y32_64x64.npz")
  ...
  def sample_observations_from_factors_no_color(self, factors, random_state):
    ...
    #Note: We sample remaining factors to be constant. We want only progressive changes.
    all_factors = self.state_space.sample_all_factors_constant(factors, random_state)
...
class ColorDSprites(DSprites):
  ...
  def __init__(self, data_path, latent_factor_indices=None):
    DSprites.__init__(self, data_path, latent_factor_indices)
...
class AbstractDSprites(DSprites):
  ...
  def __init__(self, data_path):
    ...
    DSprites.__init__(self, data_path, [1, 2, 4, 5])
    self.data_shape = [64, 64, 3]
