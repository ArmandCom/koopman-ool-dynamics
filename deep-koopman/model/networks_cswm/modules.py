...
class _EncoderCNNLarge(nn.Module):
    """CNN encoder, maps observation to obj-specific feature maps."""
    
    def __init__(self, input_dim, hidden_dim, num_objects, act_fn='sigmoid',
                 act_fn_hid='relu'):
        super(_EncoderCNNLarge, self).__init__()
...
class EncoderCNNLarge(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, output_dim, num_objects, map_size=[64, 64]):
        super(EncoderCNNLarge, self).__init__()

        self.enc = nn.Sequential(
            nn.Conv2d(input_dim+4, 16, 4, 2, 1),
            nn.CELU(),
            nn.GroupNorm(4, 16),
        ...
        x_l = torch.linspace(0, 1, map_size[0])
        y_l = torch.linspace(0, 1, map_size[1])
        x_r = x_l.flip(0)
        y_r = y_l.flip(0)
    ...
    def forward(self, obs):
        ...
        obs = self._add_positional_encoding(obs)

        img_enc = self.enc(obs)
...
class EncoderMLP(nn.Module):
    ...
    def forward(self, ins):
        h_flat = ins.view(ins.shape[0], self.num_objects, self.input_dim)
