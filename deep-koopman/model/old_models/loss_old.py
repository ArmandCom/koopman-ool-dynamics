...
def embedding_loss(output, target, epoch_iter, n_iters_start=3, lambd=0.3):
    ...
    #  passar distribucio i sample. Distribucio es calcula abans.
    ...
    kl_loss = 0
    prior = Normal(torch.zeros(1).to(mu[0].device), torch.ones(1).to(mu[0].device))
    for (m, lv) in zip(mu, logvar):
        # prior = Normal(torch.zeros_like(m), torch.ones_like(lv))
        post = Normal(m, lv)
        kl_loss = kl_loss + kl_divergence(post, prior).sum(-1).mean()
