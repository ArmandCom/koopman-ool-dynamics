...
def embedding_loss(output, target, epoch_iter, n_epochs_start=0, lambd=0.3):
    ...
    prior_pres_prob = linear_annealing(rec.device, epoch_iter[1], start_step=2000, end_step=40000, start_value=0.3, end_value=0.1)
    ...
    if lambd_hrank != 0:
        ...
        h_rank_loss = torch.norm(A, p='nuc', dim=[-2, -1]).sum()
