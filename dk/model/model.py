...
class RecKoopmanModel(BaseModel):
    ...
    def forward(self, input, epoch_iter):
        ...
        start_step = 1 # g and u must be aligned!!
        ...
        #TODO: output a confidence value with S_for_pred, instead of the rec confidence value.
        S_for_pred, confi_for_pred, G_for_pred, u_tp, sel_tp, selectors_for_pred = self.koopman.simulate(T=T_sim, g=init_g, inputs=u_sim, temp=temp, logit_out=logit_out, with_u=self.with_u) #[B, N, T - n_ts - start_step, -1]
        ...
        cases.append({#"obs": G_for_pred,
                      "obs": None,
                      "pose": S_for_pred,
                "confi": confi_for_pred, #confi[:, :, -S_for_pred.shape[2]:],
        ...
        for idx, case in enumerate(cases):
            ...
            # Note: linear
            # pose = self.linear_f_dyn_post(f_dyn_out.reshape(bs * self.n_objects * case["T"], -1, self.n_timesteps)[..., 0]).tanh()
