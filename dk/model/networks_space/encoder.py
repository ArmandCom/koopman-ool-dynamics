...
class ImageEncoder(nn.Module):
    ...
    def __init__(self, in_channels, feat_cte_dim, feat_dyn_dim, resolution, n_objects, ngf, image_size, n_timesteps, cte_app=False, bs=40):
        ...
        if image_size[0] == 320 and image_size[1] == 480:
            ...
            layers = [nn.Conv2d(in_channels + 4, 16, 4, 2, 1), # 64, 64
                      ...
                      nn.CELU(),
                      nn.GroupNorm(16, 128),
                      nn.Conv2d(128, 128, 4, 2, 1), # 16, 16
                      nn.CELU(),
                      nn.GroupNorm(16, 128),
                      nn.Conv2d(128, 256, 4, 2, 1),
                      nn.CELU(),
                      nn.GroupNorm(32, 256),
                      nn.Conv2d(256, last_hidden_dim, 3, 1, 1),
                      nn.CELU(),
                      nn.GroupNorm(16, last_hidden_dim)]

        elif image_size[0] == 160 and image_size[1] == 240:

            layers = [nn.Conv2d(in_channels + 4, 16, 4, 2, 1),
                      nn.CELU(), # use ReLU
                      # nn.BatchNorm2d(16),
                      nn.GroupNorm(4, 16),
                      nn.Conv2d(16, 32, 4, 2, 1),
    ...
    # def load_states_replica(self, *args):
    #     states = [self.states[arg].clone() for arg in args]
    #     return states if len(states) > 1 else states[0]
    ...
    def save_states_mean(self, **kwargs):
        for kw, arg in kwargs.items():
            self.states[kw].copy_(arg.data[:, -1].mean(0)[None])
    ...
    def forward(self, x, dyn_feat_input=None, block='backbone', warmup = True):
        bs, T, ch, h, w = x.shape
        ...
        if block == 'dyn_track':
            ...
            # Initialize states for all batch
            # if self.states['h_o_prev'].shape[0] == 1:
            #     self.states['h_o_prev'] = self.states['h_o_prev'].to(x.device)
            #     self.states['y_e_prev'] = self.states['y_e_prev'].to(x.device)

            x = torch.cat([x, self.pos_enc.repeat(bs, T, 1, 1, 1)], dim=2)
            x = self.cnn_attn(x.reshape(-1, *x.shape[2:])) # TODO: Encode with lateral encoder ? #convolutional encoder
            x = x.reshape(bs, T, -1, x.shape[-1]*x.shape[-2]).permute(0, 1, 3, 2) # bs, T, hw, c_cnn
            h_o_prev, y_e_prev = self.load_states('h_o_prev', 'y_e_prev')
            # h_o_prev, y_e_prev = torch.randn(1, self.n_objects, self.dim_h_o), torch.randn(1, self.n_objects, self.dim_y_e)
            if warmup:
                t_wu = T//3
                x_wu = x[:, :t_wu].flip(dims=[1])
                x = torch.cat([x_wu, x], dim=1)

            h_o_seq, y_e_seq, y_l_seq, y_p_seq, Y_s_seq, Y_a_seq = self.tracker(h_o_prev.repeat_interleave(bs, dim=0).to(x.device), y_e_prev.repeat_interleave(bs, dim=0).to(x.device), x)
            # output: rnn latent vector, y_e doesnt matter, y_l layers, y_p pose, shape, app
            self.save_states_mean(h_o_prev=h_o_seq, y_e_prev=y_e_seq)

            if warmup:
                y_p_seq = y_p_seq[:, t_wu:]
                Y_s_seq = Y_s_seq[:, t_wu:]
                Y_a_seq = Y_a_seq[:, t_wu:]
                y_e_seq = y_e_seq[:, t_wu:]
                y_l_seq = y_l_seq[:, t_wu:]
        ...
        return pose, shape, app, confi, layers