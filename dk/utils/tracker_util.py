...
import utils.tracker_base_util as F
...
class STGumbelSigmoid(nn.Module):
    ...
    def forward(self, mu, tao=None):
        if tao is None:
            tao = self.tao
        log = self.log
        u1 = torch.rand(mu.size()).to(mu.device)#.cuda()
        u2 = torch.rand(mu.size()).to(mu.device)#.cuda()
        a = (log(mu) - log(-log(u1)) - log(1 - mu) + log(-log(u2))) / tao
...
class STGumbelSoftmax(nn.Module):
    ...
    def forward(self, mu):
        log = self.log
        u = torch.rand(mu.size()).to(mu.device)#.cuda() # N * K
        # mu = CheckBP('mu')(mu)
...
class GaussianSampler(nn.Module):
    ...
    def forward(self, mu, log_var):
        standard_normal = torch.randn(mu.size()).to(mu.device)#.cuda()
