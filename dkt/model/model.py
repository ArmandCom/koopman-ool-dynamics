...
class RecKoopmanModel(BaseModel):
    ...
    def forward(self, input, epoch = 1):
        ...
        if self.composed_systems:
            ...
            A, A_inv, AA_inv = self.koopman.get_A_Ainv()
            B = None
        ...
        print(T_inp-free_pred-1, g.flip(1)[:,free_pred+1:].shape[1])
        G_for_pred_rev =    self.koopman.simulate(T=T_inp-free_pred-1, g=g.flip(1)[:,free_pred:],  u=u.flip(1)[:,free_pred:], A=A, B=B, backward=True) # Check
        output["obs_rec_pred_rev"] = G_for_pred_rev.reshape(bs * self.n_objects, -1, G_for_pred_rev.shape[-1])
        ...
        for idx, case in enumerate([rec, pred, pred_rev]):
